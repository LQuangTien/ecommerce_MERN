...
exports.statistic = async (req, res) => {
    try {
        ...
        const [total7day, totalPerMonth, top5] =
            await Promise.all([totalOrderPrice7DayLatest(), totalOrderPricePerMonthByYear(2021),
            top5HotProduct()])
...
async function top5HotProduct() {
    const products = await Product.find({}).sort({ quantitySold: -1 }).limit(5).exec();
