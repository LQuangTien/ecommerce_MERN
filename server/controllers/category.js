...
updateProductBaseOnCategoryUpdate = async (oldCategory, newCategory) => {
   ...
   const diffInNormalField = findDiffInCategoryField(oldCategory.normalField, newCategory.normalField);
   const diffInFilterField = findDiffInCategoryField(oldCategory.filterField, newCategory.filterField);

   if (diffInNormalField.deleted.length > 0 && diffInFilterField.deleted.length > 0) {
      const removeFieldFromProduct = await Product.updateMany({ category: oldCategory.name },
         {
      ...
      const removeFieldFromProduct = await Product.updateMany({ category: oldCategory.name },
         { $pull: { categoryInfo: { name: { $in: diffInNormalField.deleted } } } }, { multi: true })
   } else {
      const removeFieldFromProduct = await Product.updateMany({ category: oldCategory.name },
         { $pull: { categoryInfo: { name: { $in: diffInFilterField.deleted } } } }, { multi: true })
   }

   if (diffInNormalField.updated.length > 0 || diffInFilterField.updated.length > 0) {
...
findDiffInCategoryField = (oldFieldCategory, newFieldCategory) => {
   ...
   while (oldIndex < getIdAndNameFromOld.length) {
      ...
      //khi đó new sẽ duyệt hết mảng trước còn old thì vẫn còn item chưa duyệt nên phải xử lí không sẽ OutOfIndex
      //VD: old =[1,2,3,4,5], new=[1,2,3]
      if (newIndex === getIdAndNameFromNew.length && oldIndex < getIdAndNameFromOld.length) {
         //đẩy tất cả oldItem còn lại vào deleted vì duyệt hết new rồi mà old vẫn còn thì đây là những thằng bị xóa
         while (oldIndex < getIdAndNameFromOld.length) {
